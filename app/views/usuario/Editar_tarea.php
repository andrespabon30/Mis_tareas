...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    $stmt = $db->prepare("UPDATE tareas SET titulo=?, descripcion=?, estado=?, categoria=?, prioridad=?, fecha_vencimiento=? WHERE id=? AND usuario_id=?");
    $stmt->execute([$titulo, $descripcion, $estado, $categoria, $prioridad, $fecha_vencimiento, $id, $_SESSION['usuario_id']]);
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .card-modern { background: white; border-radius: 20px; box-shadow: 0 20px 60px rgba(0,0,0,0.3); overflow: hidden; margin-top: 50px; }
        .card-header-modern { background: linear-gradient(135deg, #4361ee, #3f37c9); color: white; padding: 25px 30px; }
        .card-header-modern h2 { margin: 0; font-size: 1.8rem; }
        .card-header-modern p { margin: 5px 0 0; opacity: 0.9; }
        ...
        .btn-modern { display: inline-block; padding: 12px 25px; border-radius: 50px; text-decoration: none; font-weight: 500; transition: all 0.3s ease; border: none; cursor: pointer; font-size: 1rem; font-family: inherit; }
        ...
        .button-group .btn-modern { flex: 1; text-align: center; }
        @media (max-width: 768px) { .card-header-modern { padding: 20px; } .card-body-modern { padding: 20px; } .button-group { flex-direction: column; } }
    </style>
</head>
<body>
    <div class="container-custom">
        <div class="card-modern">
            <div class="card-header-modern">
                ...
                <p>Actualiza los detalles de tu tarea</p>
            </div>
            <div class="card-body-modern">
                <form method="POST">
                    <div class="form-group">
                        <label><i class="fas fa-heading"></i> Título *</label>
                        <input type="text" name="titulo" value="<?php echo htmlspecialchars($tarea['titulo']); ?>" class="form-control" required>
                    </div>
                    
                    <div class="form-group">
                        <label><i class="fas fa-align-left"></i> Descripción</label>
                        <textarea name="descripcion" class="form-control"><?php echo htmlspecialchars($tarea['descripcion']); ?></textarea>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="categoria" class="form-control">
                            ...
                            <option value="Urgente" <?php echo $tarea['categoria'] == 'Urgente' ? 'selected' : ''; ?>>⚠️ Urgente</option>
                            <option value="General" <?php echo $tarea['categoria'] == 'General' ? 'selected' : ''; ?>>📋 General</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="fecha_vencimiento" value="<?php echo $tarea['fecha_vencimiento']; ?>" class="form-control">
                    </div>
